...
const sendEmailRegister2 = (to, url, text) => {
  oauth2client.setCredentials({
    refresh_token: G_REFRESH_TOKEN,
  });
  const accessToken2 = oauth2client.getAccessToken2();
  ...
  smtpTransport.sendMail2(mailOptions, (err, info) => {
    if (err) return { err };
    return info;
  });
};

const sendEmailReset2 = (to, url, text, name) => {
  oauth2client.setCredentials({
    refresh_token: G_REFRESH_TOKEN,
  });
  const accessToken2 = oauth2client.getAccessToken2();
  ...
  smtpTransport.sendMail2(mailOptions2, (err, info) => {
    if (err) return { err };
    return info;
  });
};

module.exports = { sendEmailRegister2, sendEmailReset2 };
